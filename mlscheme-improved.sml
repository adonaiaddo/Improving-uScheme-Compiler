...
(* takes an expression and a variable name, and returns true if the name is used
  as a free variable in the expression *)
(* freeIn : exp -> name -> bool *)
fun freeIn (LITERAL v)              name = false
  | freeIn (VAR (x))                name = x = name
  | freeIn (LAMBDA  ([], exp1))     name = freeIn exp1 name
  | freeIn (LAMBDA  (xs, exp1))     name = (freeIn exp1 name) andalso
                                            (not(List.exists (fn(a) => name = a)
                                                          xs))
  | freeIn (WHILEX (exp1, exp2))    name = List.exists (fn (e) => freeIn e name)
                                                          [exp1, exp2]
  | freeIn (IFX (exp1, exp2, exp3)) name = List.exists (fn (e) => freeIn e name)
                                                          [exp1, exp2, exp3]
  | freeIn (BEGIN   (xs))           name = foldr (fn (a, b) => (freeIn a name)
                                                          orelse b) false xs
  | freeIn (SET (x, e))             name = x = name orelse (freeIn e name)
  | freeIn (APPLY (e, []))          name = false
  | freeIn (APPLY (e, es))          name = (freeIn e name) orelse
                                            (foldr (fn (a, b) => (freeIn a name)
                                                          orelse b) false es)
  | freeIn (LETX (_, [], exp))      name = freeIn exp name
  | freeIn (LETX (LET, b::bs, exp)) name = freeIn (LETX (LET, [b], exp)) name
  | freeIn (LETX (LETSTAR, b::bs, exp)) name =
                          freeIn (LETX (LET, [b], LETX (LETSTAR, bs, exp))) name
  | freeIn (LETX (LETREC, b::bs, exp))  name = if not (name = (fst b)) then
                          freeIn (LETX(LETREC, [b], exp)) name else false

  (* Unit Tests *)
  local
   val set_x1 = SET ("x", LITERAL (NUM 1))
   (* More local definitions can go here for unit testing... *)
    val set_a1 = SET ("a", VAR "hello")
    val set_b1 = SET ("b", VAR "")
    
    in

    val () =
        Unit.checkAssert "set literal test"
        (fn () => freeIn set_x1 "x")

    (* More unit tests go here ... *)
    val () =
      Unit.checkExpectWith Unit.boolString "set literal var true test"
      (fn () => freeIn set_b1 "b")
      true
    ...
    val () =
      Unit.checkExpectWith Unit.boolString "set literal var false test"
      (fn () => freeIn set_b1 "a")
      false
    ...
    val () =
      Unit.checkAssert "part A with free var"
      (fn () => freeIn (LAMBDA (["m"], LAMBDA (["n"],
      APPLY (VAR "equal?", [VAR "m", VAR "n"])))) "equal?")

    val () =
      Unit.checkAssert "part B a free var"
      (fn () => freeIn (LAMBDA (["2"],
      APPLY (VAR "equal?", [VAR "1", VAR "2"]))) "equal?")
    
    val () =
      Unit.checkAssert "part A in exp not a free var"
      (fn () => not (freeIn (LAMBDA (["a"], LAMBDA (["b"],
      APPLY (VAR "equal?", [VAR "a", VAR "b"])))) "a"))
    
    val () =
      Unit.checkAssert "part B in exp not a free var"
      (fn () => not (freeIn (LAMBDA (["birthday"],
      APPLY (VAR "equal?", [VAR "happy", VAR "birthday"]))) "birthday"))


(* improve takes in a pair of lambda components and an env and returns a
  pair that contains only free vars of the lambda *)
 (* improve : (name list * exp) * 'a env -> (name list * exp) * 'a env *)
fun improve ((bs, exp), env) = ((bs, exp),
            List.filter (fn (x) => freeIn exp (fst x)) env)
